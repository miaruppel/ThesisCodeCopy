...
class LSQgridded(GriddedReconModel):
    ...
    def reconstruct(self, data: np.ndarray, traj: np.ndarray) -> np.ndarray:
        ...
        #np.save('reconVol3_postgrid.npy', reconVol)
        time_end = time.time()
        ...
        if self.crop:
            ...
            #np.save('reconVol4_postgrid.npy', reconVol)
        if self.deapodize:
            if self.verbosity:
            ...
            #np.save('reconVol5_postgrid.npy', reconVol)
            if self.verbosity:
